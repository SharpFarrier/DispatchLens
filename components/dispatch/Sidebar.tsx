...
import { LogOut, LayoutGrid, Warehouse as WarehouseIcon, Settings as SettingsIcon, ChevronDown, ChevronRight } from 'lucide-react'
...
export default function Sidebar({ items, tab, setTab, username, onSignOut }: {
  ...
  const sectionOf = (k: string): NavItem['section'] => visible.find(i => i.key === k)?.section ?? 'orders'

  // Accordion: one section open at a time; default = the section holding the current tab.
  const [openSection, setOpenSection] = useState<NavItem['section']>(sectionOf(tab))
  // If the tab changes to one in a different section (e.g. via mobile / search), open that section.
  useEffect(() => { setOpenSection(sectionOf(tab)) /* eslint-disable-next-line */ }, [tab])

  const [mobileSection, setMobileSection] = useState<NavItem['section']>(sectionOf(tab))

  const itemBtn = (i: NavItem) => {
    ...
    return (
      <button key={i.key} onClick={() => setTab(i.key)} style={{
        ...
        padding: '7px 12px 7px 34px', borderRadius: 8, border: 'none', cursor: 'pointer',
        background: active ? 'var(--accent-solid)' : 'transparent',
        color: active ? '#fff' : 'var(--sidebar-muted)',
        fontSize: 13, fontWeight: active ? 600 : 500, fontFamily: 'var(--font-sans)',
  ...
  const sectionHeader = (sec: typeof SECTIONS[number]) => {
    const list = bySection(sec.key)
    if (list.length === 0) return null
    const isOpen = openSection === sec.key
    const hasActive = list.some(i => i.key === tab)
    return (
      <div key={sec.key}>
        <button onClick={() => setOpenSection(isOpen ? ('' as NavItem['section']) : sec.key)} style={{
          display: 'flex', alignItems: 'center', gap: 10, width: '100%', textAlign: 'left' as const,
          padding: '9px 12px', borderRadius: 8, border: 'none', cursor: 'pointer',
          background: 'transparent', color: hasActive && !isOpen ? '#fff' : 'var(--sidebar-muted)',
          fontSize: 13.5, fontWeight: 600, fontFamily: 'var(--font-sans)',
        }}
          onMouseEnter={e => { e.currentTarget.style.background = 'var(--sidebar-hover)' }}
          onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}>
          <span style={{ display: 'flex', color: hasActive ? 'var(--accent)' : 'var(--sidebar-muted)' }}>{sec.icon}</span>
          <span style={{ flex: 1 }}>{sec.label}</span>
          {isOpen ? <ChevronDown size={15} /> : <ChevronRight size={15} />}
        </button>
        {isOpen && <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 2, marginTop: 2, marginBottom: 4 }}>{list.map(itemBtn)}</div>}
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <aside className="dl-sidebar" style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto' as const, display: 'flex', flexDirection: 'column' as const, gap: 2 }} className="no-scrollbar">
          {sectionHeader(SECTIONS[0])}
          {sectionHeader(SECTIONS[1])}
        </div>

        {/* Settings + account pinned bottom */}
        <div style={{ borderTop: '1px solid var(--sidebar-border)', paddingTop: 8, marginTop: 6 }}>
      ...
      {/* ── Mobile: section bottom bar + sub-view strip (unchanged) ── */}
